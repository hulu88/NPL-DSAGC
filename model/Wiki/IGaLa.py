...
def trainer(x, y, A, adj_label):
    ...
    A=kneighbors_graph(z,args.k,mode='connectivity',include_self=False)
    A=A.toarray()
    f_adj=0.5*(A+A.T)
    ...
    for epoch in range(args.max_epoch):
        ...
        if epoch%args.update_interval==0:
            _,z=model(x,asm,asp)
            z=F.normalize(z,p=2,dim=1)
            f_adj=torch.mm(z,z.t())

            A=kneighbors_graph(z.detach().numpy(),args.k,mode='connectivity',include_self=False)
            A=A.toarray()
            A=0.5*(A+A.T)
        ...
        adj=kneighbors_graph(z.detach().numpy(),args.k,mode='connectivity',include_self=False)
        adj=adj.toarray()
        adj=0.5*(adj+adj.T)
        cluster=SpectralClustering(n_clusters=args.n_clusters,affinity='precomputed',random_state=0)
        y_pred=cluster.fit_predict(adj)
